...
def plot_data_article(extracted_data, joint_to_plot, movement_to_plot=None):
        ...
        if article != "Kolz et al. 2020" and article != "Bourne 2003":
            ...
            for j, humeral_motion in enumerate(list_humeral_motion):
                for ind_dof in range(3):
                    ...
                    data_to_plot_humeral_motion = data_to_plot_article.loc[
                        data_to_plot_article["humeral_motion"] == humeral_motion
                    ]
                    data_to_plot_humeral_motion = data_to_plot_humeral_motion.loc[
                        data_to_plot_humeral_motion["degree_of_freedom"] == str(ind_dof + 1)
                    ]

                    ax[0, ind_dof].plot(
                        data_to_plot_humeral_motion["humerothoracic_angle"],
                        data_to_plot_humeral_motion["value"] * 180 / np.pi,
                        color=list_color[i],
                        marker=dict_marker[humeral_motion],
                        linestyle="None",
                    )
    ...
    if movement_to_plot is None:
        ...
        for name_movement in list_all_movement:
            ax[0, 2].plot(
                [], [], label=name_movement, color="black", marker=dict_marker[name_movement], linestyle="None"
            )
